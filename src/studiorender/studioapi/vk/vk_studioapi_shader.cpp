...
{
	hash_t	hash = FastHash( shaderReflectionConstnatBuffer.name.c_str(), ( uint64 )shaderReflectionConstnatBuffer.name.size() * sizeof( achar ) );
...
{
	hash_t	hash = FastHash( shaderReflectionStorageBuffer.name.c_str(), ( uint64 )shaderReflectionStorageBuffer.name.size() * sizeof( achar ) );
...
{
	hash_t	hash = FastHash( shaderReflectionImageSampler.name.c_str(), ( uint64 )shaderReflectionImageSampler.name.size() * sizeof( achar ) );
...
==================
*/
CStudioAPIShaderVk::CStudioAPIShaderVk( studioAPIShaderType_t type, const achar* pEntryPointName, const byte* pBytecode, uint64 bytecodeSize, const byte* pReflectionData, uint64 reflectionDataSize, const achar* pDebugName /* = "" */ )
	: type( type )
