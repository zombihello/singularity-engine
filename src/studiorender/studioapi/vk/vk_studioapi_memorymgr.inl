...
FORCEINLINE void CStudioAPIMemoryMgrVk::FreeResource( TFreeLambda&& freeLambda ) const
{
	...
	AssertMsg( currentCmdBuffer.pWritePtr - currentCmdBuffer.pBuffer + sizeof( pendingFreeResourceFn_t ) + sizeof( uint32 ) + sizeof( TFreeLambda ) < currentCmdBuffer.size, "Not enough memory in the command buffer to write the command" );
