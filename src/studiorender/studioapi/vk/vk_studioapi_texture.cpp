...
void VK_GetUsedQueueFamiliesFromTextureUsage( uint32 usageFlags, uint32& graphicsQueueFamilyIndex, uint32& computeQueueFamilyIndex, uint32& transferQueueFamilyIndex )
{
	...
	if ( usageFlags & STUDIOAPI_TEXTURE_USAGE_FLAG_TEXTURE ||
		 usageFlags & STUDIOAPI_TEXTURE_USAGE_FLAG_RENDER_TARGET ||
		 usageFlags & STUDIOAPI_TEXTURE_USAGE_FLAG_UNORDERED_ACCESS )
	{
	...
	if ( usageFlags & STUDIOAPI_TEXTURE_USAGE_FLAG_TRANSFER_SRC ||
		 usageFlags & STUDIOAPI_TEXTURE_USAGE_FLAG_TRANSFER_DST ||
		 !( usageFlags & STUDIOAPI_TEXTURE_USAGE_FLAG_LINEAR ) )
	{
...
==================
*/
CStudioAPISamplerVk::CStudioAPISamplerVk( const studioAPISamplerCreateInfo_t& createInfo, const achar* pDebugName /* = "" */ )
...
==================
*/
CStudioAPITextureVk::CStudioAPITextureVk( studioAPITextureType_t type, uint32 sizeX, uint32 sizeY, uint32 sizeZ, uint32 numLayers, uint32 numMips, uint32 usageFlags, studioAPIPixelFormat_t pixelFormat, const byte* pData /* = NULL */, const achar* pDebugName /* = "" */ )
	...
	, pStudioAPIVkShutdownDelegate( NULL )
{
	...
	if ( ( usageFlags & STUDIOAPI_TEXTURE_USAGE_FLAG_RENDER_TARGET ) && !VK_IsDepthPixelFormat( pixelFormat ) )
	{
		...
		if ( vkImageViewCreateInfo.viewType == VK_IMAGE_VIEW_TYPE_CUBE ||
			 vkImageViewCreateInfo.viewType == VK_IMAGE_VIEW_TYPE_CUBE_ARRAY )
		{
