...
{
	return constantBuffersDict.empty() && storageBuffersDict.empty() && sampledImagesDict.empty() && storageImagesDict.empty() && separateTexturesDict.empty() && separateSamplersDict.empty();
}
...
FORCEINLINE bool CShaderReflection::IsFreeBindingIndex( uint32 bindingIndex, uint32 descriptorSetIndex ) const
{
	...
	return shaderDesciptorSet.constantBuffersDict.find( bindingIndex ) == shaderDesciptorSet.constantBuffersDict.end() && shaderDesciptorSet.storageBuffersDict.find( bindingIndex ) == shaderDesciptorSet.storageBuffersDict.end() && shaderDesciptorSet.sampledImagesDict.find( bindingIndex ) == shaderDesciptorSet.sampledImagesDict.end() && shaderDesciptorSet.storageImagesDict.find( bindingIndex ) == shaderDesciptorSet.storageImagesDict.end() && shaderDesciptorSet.separateTexturesDict.find( bindingIndex ) == shaderDesciptorSet.separateTexturesDict.end() && shaderDesciptorSet.separateSamplersDict.find( bindingIndex ) == shaderDesciptorSet.separateSamplersDict.end();
}
