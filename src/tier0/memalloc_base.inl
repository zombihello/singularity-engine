...
{
	return S_Max<uint32>( numBytes >= DEFAULT_ALIGNMENT ? DEFAULT_ALIGNMENT : MIN_ALIGNMENT, alignment );
}
...
FORCEINLINE void CMemAllocBase::OutOfMemory( size numBytes, uint32 alignment ) const
{
	...
	Sys_Error( "Ran out of memory allocating %llu bytes (numBytes: %llu, alignment: %u)", S_Align<size>( numBytes, alignment ), numBytes, alignment );
}
